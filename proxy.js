...
const enableMaintenance = () => {
  ...
  console.log(
    `document.cookie="temp=${maintenance};domain=.filegarden.com;path=/;expires="+new Date(Date.now()+1000*60*60*24*7).toUTCString();document.cookie="temp=${maintenance};domain=file.garden;path=/;expires="+new Date(Date.now()+1000*60*60*24*7).toUTCString();`,
  );
};
...
const listener = (req, res) => {
  if (
    maintenance &&
    !req.headers.host?.endsWith(".gg") &&
    !req.headers.host?.endsWith(".gold") &&
    !(req.headers.cookie && req.headers.cookie.includes(maintenance))
  ) {
    res
      .writeHead(req.method === "OPTIONS" ? 200 : 503, {
  ...
  if (req.headers.host) {
    if (
      req.headers.host.endsWith(".garden") ||
      req.headers.host === "pipe.miroware.io"
    ) {
      target = "http://localhost:" + (8200 + Math.floor(Math.random() * 4));
    } else if (
      req.headers.host.endsWith(".at") ||
      req.headers.host.endsWith(".gg")
    ) {
