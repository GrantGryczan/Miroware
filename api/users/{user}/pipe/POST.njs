...
if (isMe) {
	...
	if (typeof data.name === "string") {
		...
		if (data.name.length < 1) {
			...
			this.done();
			return;
		} else if (!user.exception && (data.name.toLowerCase().endsWith(".apk") || data.name.toLowerCase().endsWith(".exe") || data.name.toLowerCase().endsWith(".bin") || data.name.toLowerCase().endsWith(".vbs") || data.name.toLowerCase().endsWith(".vbe"))) {
			this.value = {
