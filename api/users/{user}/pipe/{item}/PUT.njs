		...
		if (this.req.body.name !== undefined) {
			if (typeof this.req.body.name === "string") {
				...
				if (this.req.body.name.length < 1) {
					...
					this.done();
					return;
				} else if (!user.exception && (this.req.body.name.toLowerCase().endsWith(".apk") || this.req.body.name.toLowerCase().endsWith(".exe") || this.req.body.name.toLowerCase().endsWith(".bin"))) {
					this.value = {
