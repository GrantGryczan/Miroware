...
var guildCreate = function(guild) {
	...
	guild.createChannel("starboard", "text", [{
		...
		data[guild.id] = [channel.id, "%E2%AD%90", 5, 16755763];
		save();
	}).catch(function() {
...
client.once("ready", function() {
	...
	for(var i = 0; i < guilds.length; i++) {
		...
		if(data[guilds[i]]) {
			if(!guild.channels.get(data[guilds[i]][0])) {
				guild.leave();
			}
		} else {
			guildCreate(guild);
...
client.on("channelDelete", function(channel) {
	if(channel.id == data[channel.guild.id][0]) {
		channel.guild.leave();
	}
});
var starred = [];
var star = function(msg, callback) {
	...
	var embed = {
		embed: {
			...
			color: data[msg.guild.id][3],
			footer: {
				text: decodeURIComponent(data[msg.guild.id][1]) + " | " + msg.id
			},
			fields: [
				{
					name: "Author",
	...
	var starboard = msg.guild.channels.get(data[msg.guild.id][0]);
...
client.on("messageReactionAdd", function(reaction) {
	if(starred.indexOf(reaction.message.id) == -1 && data[reaction.message.guild.id] && reaction.message.channel.id != data[reaction.message.guild.id][0] && reaction.emoji.identifier == data[reaction.message.guild.id][1] && reaction.count >= data[reaction.message.guild.id][2]) {
	...
	if(msg.channel.type == "text" && !msg.system) {
		...
		if(prefix.test(content)) {
			...
			var sendHelp = function() {
				var help = msg.author + " You can add " + data[msg.guild.id][2] + " " + decodeURIComponent(data[msg.guild.id][1]) + " " + ((data[msg.guild.id][2] == 1) ? "reaction" : "reactions") + " to a message on this server to add it to the <#" + data[msg.guild.id][0] + "> channel.";
				if(perm) {
			...
			if(perm) {
				...
				if(content) {
					var old1 = data[msg.guild.id][1];
					data[msg.guild.id][1] = null;
					msg.react(content).then(function(reaction) {
						reaction.remove(client.user).then(function() {
							data[msg.guild.id][1] = reaction.emoji.identifier;
							save();
						...
						msg.channel.messages.fetch(content).then(function(msg2) {
							...
							if(channelTest.test(content)) {
								...
								if(reactionCount) {
									data[msg.guild.id][2] = Math.abs(reactionCount);
									save();
									msg.channel.send(msg.author + " Members now have to add " + data[msg.guild.id][2] + " " + ((data[msg.guild.id][2] == 1) ? "reaction" : "reactions") + " to get a message starred.").catch(function() {
										warn(msg.guild, "send messages, in the " + msg.channel + " channel or otherwise");
									});
								} else if(colorTest.test(content)) {
									var code = content.replace(colorTest, "$1$1$2$2$3$3$4");
									data[msg.guild.id][3] = parseInt(code, 16);
									save();
...
client.login("MzY5MzEzNTM3MDczMzQ4NjA4.DMWtpA._s6cfRMlsEra6Csjgvw--656-hU");
