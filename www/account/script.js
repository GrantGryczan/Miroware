...
add.addEventListener("click", Miro.checkSuper.bind(null, () => {
	Miro.auth("Add Connection", "Authenticate a new connection for your account.", postConnection, undefined, true).then(newConnection);
}));
...
const verificationResent = Miro.response(() => {
	new Miro.Dialog("Account Verification", html`A verification email has been resent to <b>$${form.elements.email.value}</b>.`);
});
