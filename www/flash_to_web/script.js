...
const SWF = {
	...
	StyleChangeRecord: (value, shape) => {
		...
		if (value.StateFillStyle0) {
			value.FillStyle0Value = shape.FillStylesValue.FillStyles[value.FillStyle0 = SWF.UB(shape.NumFillBitsValue)];
		}
		if (value.StateFillStyle1) {
			value.FillStyle1Value = shape.FillStylesValue.FillStyles[value.FillStyle1 = SWF.UB(shape.NumFillBitsValue)];
		}
		if (value.StateLineStyle) {
			value.LineStyleValue = shape.LineStylesValue.LineStyles[value.LineStyle = SWF.UB(value.NumLineBitsValue)];
		}
