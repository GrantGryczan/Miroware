...
const SWF = {
	...
	FLOAT16: () => { // Half-precision (16-bit) floating-point number
		...
		return exponent === 16 ? (mantissa ? NaN : sign * Infinity) : sign * 2 ** exponent * (2 ** (Math.log(mantissa) / Math.log(2) - 10) + (exponent ? 1 : 0));
	},
	FLOAT: () => { // Half-precision (32-bit) IEEE Standard 754 compatible
		...
		return exponent === 128 ? (mantissa ? NaN : sign * Infinity) : sign * 2 ** exponent * (2 ** (Math.log(mantissa) / Math.log(2) - 23) + (exponent ? 1 : 0));
	},
