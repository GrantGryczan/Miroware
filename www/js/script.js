...
Miro.prepare = (node) => {
  if (!(node instanceof Element || node instanceof Document)) {
    throw new MiroError(
      "The `node` parameter must be an element or a document."
    );
  }
  ...
  for (const elem of node.querySelectorAll(
    ".mdc-ripple:not(.mdc-ripple-upgraded)"
  )) {
  ...
  for (const elem of node.querySelectorAll(
    ".mdc-checkbox:not(.mdc-checkbox-upgraded)"
  )) {
    elem
      .querySelector(".mdc-checkbox__background")
      .appendChild(checkmark.cloneNode(true));
...
Miro.value = (input) => {
  if (
    ...
    throw new MiroError(
      "The `input` parameter must be an HTML `input`, `textarea`, or `select` element."
    );
  }
...
Miro.inputState = (input, state) => {
  if (!(input instanceof HTMLInputElement)) {
    throw new MiroError(
      "The `input` parameter must be an HTML `input` element."
    );
  }
  input.disabled = !state;
  for (const mdcType of mdcTypes) {
    if (input.parentNode.classList.contains(`mdc-${mdcType}`)) {
      input.parentNode.classList[state ? "remove" : "add"](
        `mdc-${mdcType}--disabled`
      );
    }
  }
};
...
Miro.Dialog = class MiroDialog {
  constructor(title, content, buttons = ["Okay"]) {
    ...
    if (typeof content === "string") {
      ...
      throw new MiroError(
        "The `content` parameter must be a string or a DOM node."
      );
    }
    this.ready = false;
    ...
    if (buttons.length) {
      buttons = buttons.map((item) => {
        ...
        if (typeof item === "string") {
          ...
          throw new MiroError(
            "The `buttons` parameter's array must only include strings and objects."
          );
        }
    ...
    this[_promise] = new Promise((resolve) => {
      ...
      form.addEventListener(
        "submit",
        (submitted = !buttonsElem.querySelector("button[type='submit']"))
          ? (evt) => {
              evt.preventDefault();
            }
          : (evt) => {
  ...
  close(value) {
    setTimeout(() => {
      if (this.ready) {
        ...
        throw new MiroError(
          "The dialog has not finished instantiating and cannot be closed."
        );
      }
    });
  }
...
Miro.progress = mdc.linearProgress.MDCLinearProgress.attachTo(
...
const snackbar = mdc.snackbar.MDCSnackbar.attachTo(
  document.body.querySelector("#snackbar")
);
...
Miro.response = (success, failure) => async (xhr) => {
  if (Math.floor(xhr.status / 100) === 2) {
    ...
    const error =
      ...
      xhr.statusText ||
      "An unknown error occurred: " + html`$${xhr.responseText}`.textContent;
...
const apiOrigin = location.origin.includes("localhost")
...
const authFailed = (data) => {
  if (data) {
    new Miro.Dialog(
      "Error",
      (data.response && data.response.error && html`${xhr.response.error}`) ||
        data.statusText ||
        data.details ||
        data.error ||
        data
...
const clickAuth = (service) =>
  function () {
    ...
    new Promise(auths[service].bind(this))
      .then((code) => {
        try {
          if (shouldBlock) {
            Miro.block(false);
          ...
          throw new MiroError(
            "The `send` parameter must be a promise (of `Miro.request` or which resolves a `Miro.request`)."
          );
        }
      })
      .catch(catchAuth);
...
const auths = {
  ...
  Discord: (resolve, reject) => {
    const win = window.open(
      `https://discordapp.com/api/oauth2/authorize?client_id=430826805302263818&redirect_uri=${encodeURIComponent(
        location.origin
      )}%2Flogin%2Fdiscord%2F&response_type=code&scope=identify%20email`,
      "authDiscord"
    );
  ...
  password: async function (resolve, reject) {
    ...
    const content = html`
      ...
      <div class="mdc-text-field">
        <input
          ...
          type="password"
          minlength="10"
          autocomplete="${creation ? "new" : "current"}-password"
          required
        />
      ...
      content.appendChild(html`
        ...
        <div class="mdc-text-field">
          <input
            ...
            required
          />
          <label class="mdc-floating-label" for="confirmPassword"
    ...
    const dialog = new Miro.Dialog("Password", content, [
      ...
      if (value === 0) {
        if (
          !creation ||
          dialog.form.elements.password.value ===
            dialog.form.elements.confirmPassword.value
        ) {
  ...
  Miro.request("GET", "/token").then(
    Miro.response((xhr) => {
      if (xhr.response.super) {
        ...
        Miro.auth(
          "Security",
          "You must confirm the validity of your credentials before continuing.",
          putToken,
          undefined,
          false,
          success
...
if (
  (Miro.in = JSON.parse(
    document.head.querySelector("meta[name='in']").getAttribute("content")
  ))
) {
  Miro.logOut = () =>
    Miro.request("DELETE", "/token").then(Miro.response(Miro.reload));
  ...
  container.querySelector("#logOut").addEventListener("click", () => {
    new Miro.Dialog("Logout", "Are you sure you want to log out?", [
      "Yes",
      "No",
    ]).then(checkLogout);
  });
  ...
  Miro.logIn = (dest) => {
    location.href = `/login/?dest=${encodeURIComponent(
      dest ||
        location.href.slice(
          location.href.indexOf("/", location.href.indexOf("//") + 2)
        )
    )}`;
  };
}
Miro.data = JSON.parse(
  document.head.querySelector("meta[name='data']").getAttribute("content")
);
Miro.focused = () => !(drawer.open || container.querySelector(".mdc-dialog"));
Miro.typing = () =>
  container.querySelector(
    "input:not([type='button']):not([type='submit']):not([type='reset']):focus, textarea:focus"
  );
