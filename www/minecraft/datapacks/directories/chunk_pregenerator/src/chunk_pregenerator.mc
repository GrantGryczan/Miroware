...
function continue {
	...
	tellraw @s ["\n                  ",{"text":"[ Cancel ]","clickEvent":{"action":"run_command","value":"/function chunk_pregenerator:setup"},"hoverEvent":{"action":"show_text","contents":{"text":"Click to edit the setup configuration again.","color":"gray"}},"color":"red"},"                  ",{"text":"[ Confirm ]","clickEvent":{"action":"run_command","value":"/function chunk_pregenerator:confirm"},"hoverEvent":{"action":"show_text","contents":{"text":"Click to start pre-generating chunks with the current configuration above.","color":"gray"}},"color":"green"},"                  "]
	tellraw @s {"text":"                                                                                ","color":"dark_gray","strikethrough":true}
	execute store result score #sendCommandFeedback chunkPre.config run gamerule sendCommandFeedback
...
function confirm {
	...
	block {
		...
		execute unless score #steps chunkPre.config matches -1 run {
			name load_corner
			scoreboard players set #steps chunkPre.config -1
			...
			schedule 3t replace {
				name initiate
				execute as @e[type=minecraft:marker,tag=chunkPre.marker,limit=1] at @s run {
					name initiate_from_corner
					...
					block {
						...
						execute as @e[type=minecraft:marker,tag=chunkPre.edge,limit=256] run {
						...
						execute as @e[type=minecraft:marker,tag=chunkPre.notEdge,limit=256] run {
							...
							execute if score @s chunkPre.config < #length chunkPre.config at @s run {
								...
								execute positioned ~ 0 ~16 run {
									...
									scoreboard players operation @e[type=minecraft:marker,tag=chunkPre.notEdge,distance=..0.01,limit=1] chunkPre.config = @s chunkPre.config
									scoreboard players add @e[type=minecraft:marker,tag=chunkPre.notEdge,distance=..0.01,limit=1] chunkPre.config 1
									execute store result bossbar chunk_pregenerator:progress value run scoreboard players add #done chunkPre.config 1
									scoreboard players operation #percent chunkPre.config = #done chunkPre.config
									scoreboard players operation #percent chunkPre.config *= #100 chunkPre.config
									scoreboard players operation #percent chunkPre.config /= #area chunkPre.config
...
function stop {
	...
	execute if score #done chunkPre.config matches 0.. run {
		...
		schedule clear chunk_pregenerator:stop
		bossbar set chunk_pregenerator:progress players
		execute as @e[type=minecraft:marker,tag=chunkPre.marker] run {
			name remove_marker
