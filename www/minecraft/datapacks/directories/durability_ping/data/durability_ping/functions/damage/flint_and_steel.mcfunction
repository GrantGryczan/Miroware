advancement revoke @s only durability_ping:damage/flint_and_steel
scoreboard players set #durability duraPing.dummy 64
data modify storage durability_ping:storage name set value '{"translate":"item.minecraft.flint_and_steel","color":"gold"}'
scoreboard players set @s duraPing.weapon 60
execute store success score #mainhand duraPing.dummy if entity @s[nbt={SelectedItem:{id:"minecraft:flint_and_steel"}}]
...
execute if score #config duraPing.dummy matches 1 run title @s subtitle ["",{"storage":"durability_ping:storage","nbt":"name","interpret":true,"color":"gold"},{"text":" durability low! ","color":"red"},{"score":{"name":"#durability","objective":"duraPing.dummy"},"color":"gold"},{"text":" of 64 remaining.","color":"red"}]
...
execute if score #config duraPing.dummy matches 2 run title @s subtitle [{"score":{"name":"#durability","objective":"duraPing.dummy"},"color":"gold"},{"text":" of 64 remaining.","color":"red"}]
execute if score #config duraPing.dummy matches 3 run tellraw @s ["",{"storage":"durability_ping:storage","nbt":"name","interpret":true,"color":"gold"},{"text":" durability low! ","color":"red"},{"score":{"name":"#durability","objective":"duraPing.dummy"},"color":"gold"},{"text":" of 64 remaining.","color":"red"}]
execute if score #config duraPing.dummy matches 4 run title @s actionbar ["",{"storage":"durability_ping:storage","nbt":"name","interpret":true,"color":"gold"},{"text":" durability low! ","color":"red"},{"score":{"name":"#durability","objective":"duraPing.dummy"},"color":"gold"},{"text":" of 64 remaining.","color":"red"}]