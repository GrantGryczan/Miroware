...
module.exports = {
	onBuildSuccess: ({config}) => Promise.all([(async () => {
		...
		const scanDirectory = async parent => {
			...
			for (const child of await fs.readdir(parent)) {
				...
				if ((await fs.stat(childPath)).isDirectory()) {
					...
					archive.append((await fs.readFile(childPath, "utf8")).replace(/COLOR_1/g, "dark_aqua").replace(/COLOR_2/g, "aqua").replace(/COLOR_3/g, "gold"), {
						name: childPath
					});
				}
			}
		...
		const scanDirectory = async parent => {
			...
			for (const child of await fs.readdir(parent)) {
				...
				if ((await fs.stat(childPath)).isDirectory()) {
					...
					promises.push((async () => {
						await fs.writeFile(targetPath, (await fs.readFile(childPath, "utf8")).replace(/grant_gryczan/g, "vanillatweaks").replace(/COLOR_1/g, "gold").replace(/COLOR_2/g, "yellow").replace(/COLOR_3/g, "green"));
					})());
				}
			}
			await Promise.all(promises);
